...
<div class="negro">
    <div class="text-center fondo">
        <h1 class="text-uppercase color py-5">Gente que solicita informacion</h1>
    </div>
</div>
<section class="container-lg my-5">
    <div class="row tabla">
        <table class="table" id="listaConsultas">
            <thead>
                <tr class="text-center">
                    ...
                    <th class="bg-body-secondary">mensaje</th>
            ...
            <tbody>
                <?php
                    ...
                    $data = $cnx->query("SELECT id,nombre,apellidos,mail,tlf,mensaje FROM marco_contacto ORDER BY id");
                    while ($row = $data->fetch_assoc()) {
                        extract($row);
                        echo "<tr class='text-center align-middle'><td>$id</td><td>$nombre</td><td>$apellidos</td><td>$mail</td><td>$tlf</td><td>$mensaje</td><td><button type='button' class='btn btn-success btnEstado'>Estado</button></td><td><button type='button' class='btn btn-danger btnEliminar'>Eliminar</button></td></tr>";
                    }
                ?>
            </tbody>
        </table>
